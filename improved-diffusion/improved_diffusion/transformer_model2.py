    ...
    def __init__(
        ...
        if config is None:
            config_kwargs = {
                ...
                "gi_key_side": gi_key_side,
                "gi_query_side": gi_query_side,
                "gi_value_side": gi_value_side,
                "quant_m": quant_m,
                "weight_i_width": weight_i_width,
                "weight_f_width": weight_f_width,
        ...
        if experiment_mode == 'conditional_gen':
            ...
            #     "quant_m": quant_m,  #     "weight_i_width": weight_i_width,  #     "weight_f_width": weight_f_width,
        time_embed_dim = model_channels * 4
        if quant_m == 'original':
            print('*****************  original time_embed  *****************')
            self.time_embed = nn.Sequential(
                linear(model_channels, time_embed_dim),
                SiLU(),
            ...
            print('*****************  quantized time_embed  *****************')
            # self.time_embed = nn.Sequential(
            #     LinearQuant(model_channels, time_embed_dim, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            #     SiLU(),
            #     LinearQuant(time_embed_dim, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width)
            # )
            self.time_embed = nn.Sequential(
                LinearQuant(model_channels, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            )
        ...
        if quant_m == 'original':
            ...
            print('*****************  quantized input_up_proj  *****************')
            # self.input_up_proj = nn.Sequential(
            #     LinearQuant(in_channels, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            #     nn.Tanh(),
            #     LinearQuant(config.hidden_size, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width)
            # )
            self.input_up_proj = nn.Sequential(
                LinearQuant(in_channels, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            )
        ...
        if init_pretrained:
            ...
            # for name, param in temp_bert.named_parameters():
            #     print(name)
            ...
            #     if 'inhibition' in name:   #  inhibition  attention
            #         print(name)
            #         param.requires_grad = True
            # print('******************************************')
        ...
        if quant_m == 'original':
            print('*****************  original output_down_proj  *****************')
            ...
            print('*****************  quantized output_down_proj  *****************')
            # self.output_down_proj = nn.Sequential(
            #     LinearQuant(config.hidden_size, config.hidden_size, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            #     nn.Tanh(),
            #     LinearQuant(config.hidden_size, out_channels, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
            # )
            self.output_down_proj = nn.Sequential(
                LinearQuant(config.hidden_size, out_channels, bias=True, H=1, W_LR_scale="Glorot", config_q=quant_m, weight_f_width=weight_f_width, weight_i_width=weight_i_width),
                )
